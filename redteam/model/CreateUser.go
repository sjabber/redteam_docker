...
func (u *User) CreateUsers() (int, error) {
	...
	if err != nil {
		// 500에러, 예상 외의 오류발생
		SugarLogger.Errorf("db error : %v", err.Error())
		return 500, fmt.Errorf("db connection error")
	}
	defer db.Close()
	...
	if len(u.Password) < 1 || len(u.PasswordCheck) < 1 || len(u.Email) < 1 || len(u.Name) < 1 {
		// 정보를 입력해 주세요.
		return 400, fmt.Errorf("Please enter the information. ")
	}
	...
	userLookup := Email{}
	err = row.Scan(&userLookup.Email)
	if userLookup.Email != ""  {
		// 이미 존재하는 이메일입니다.
		return 403, fmt.Errorf("This account already exists. ")
	}

	// 비밀번호 길이 16자 미만
	if len(u.Password) > 17 || len(u.PasswordCheck) > 17 {
		// 비밀번호나 이메일 형식이 올바르지 않습니다.
		return 402, fmt.Errorf("Password must be 16 characters or less. ")
	}

	// 비밀번호 길이 8자 이상
	if len(u.Password) < 7 || len(u.PasswordCheck) < 7 {
		// 비밀번호나 이메일 형식이 올바르지 않습니다.
		return 402, fmt.Errorf("Password must be at least 8 characters long. ")
	}

	// 비밀번호 형식검사 검증
	if CheckPassword(u.Password) != nil {
		// 비밀번호 검증시 상황별로 에러메시지 출력
		return 402, CheckPassword(u.Password)
	}
	...
	if err != nil {
		// 500에러, 예상 외의 오류발생
		SugarLogger.Errorf("account creating error (1) : %v", err.Error())
		return 500, fmt.Errorf("There was an error creating an account. ")
	}
	...
	if err != nil {
		SugarLogger.Errorf("db error (1) : %v", err.Error())
		return 500, fmt.Errorf("A database error has occurred. (1)")
	}

	// smtp_info 테이블에 삽입될 비밀번호를 암호화한다.
	block, err := aes.NewCipher(key) // key 값을 블록에 집어넣는다.
	if err != nil {
		// 500에러, 예상 외의 오류 발생
		SugarLogger.Errorf("account creating error (2) : %v", err.Error())
		return 500, err
	}
	...
	if err != nil {
		SugarLogger.Errorf("db error (2) : %v", err)
...
func Encrypt(b cipher.Block, plaintext []byte) []byte {
	...
	mode.CryptBlocks(ciphertext, plaintext) // 암호화 블록 mode 인스턴스로 암호화, plaintext 는 ciphertext 의 배수
...
func Decrypt(b cipher.Block, ciphertext []byte) []byte {
	...
	if len(ciphertext)%aes.BlockSize != 0 {
		...
		SugarLogger.Error("The length of decrypted data must be a multiple of the block size. ")
		return nil
	}
	...
	mode.CryptBlocks(plaintext, ciphertext) // 복호화 블록 mode 인스턴스로 복호화, ciphertext 는 plaintext 의 배수
